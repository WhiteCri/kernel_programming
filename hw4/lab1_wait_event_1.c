/* **************** CLDD:1.0 s_19/lab1_wait_event_1.c **************** */
/*
...
static DECLARE_WAIT_QUEUE_HEAD (wq); //static waitqueue declare

static atomic_t data_ready; //set to 0 as module is openned
...
mycdrv_read (struct file *file, char __user * buf, size_t lbuf, loff_t * ppos)
{
    printk (KERN_INFO "process %i (%s) going to sleep\n", current->pid,
            current->comm);
    wait_event_interruptible (wq, (atomic_read (&data_ready))); //when read() is called, the process sleeps
              ...
              loff_t * ppos)
{
    ...
    wake_up_interruptible (&wq);//when write is called, the read processes wake up
    printk (KERN_INFO "after wake_up_interruptible() ...\n");
    return nbytes;
}
...
MODULE_DESCRIPTION ("CLDD:1.0 s_19/lab1_wait_event_1.c");
