/* **************** CLDD:1.0 s_19/lab1_wait_event_many.c **************** */
/*
...
/*
 *  Using Wait Queues (wait_for_completion(), all sleepers wake up solution)
 *
...
mycdrv_read (struct file *file, char __user * buf, size_t lbuf, loff_t * ppos)
{
    printk (KERN_INFO "process %i (%s) going to sleep\n", current->pid,
            current->comm);
    atomic_inc (&nsleepers); //incremeted multiple times as many processes call read() 
    ...
    if (atomic_dec_and_test (&nsleepers))
        atomic_set (&data_ready, 0);//must be set to zero. If not, then another process which calls read()
	//won't wait passing wait_event_interruptible() 
...
MODULE_DESCRIPTION ("CLDD:1.0 s_19/lab1_wait_event_many.c");
