...
static unsigned long delay_sum, delay_max, delay_min, delay, j_pre;
int counter = 0;
	static void t_fun(unsigned long t_arg)
{
	...
	if (delay_min > delay) delay_min = delay;
	delay_sum += delay;
	
	if (counter % 100 == 0){
		pr_info("min_delay, max_delay, avg_delay: %ld %ld %ld\n",
			delay_min, delay_max, delay_sum/counter);
	}
