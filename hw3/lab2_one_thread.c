...
static irqreturn_t my_interrupt(int irq, void *dev_id)
{
	...
	wake_up_interruptible(&wq);//wake up all interruptible thread
...
static int thr_fun(void *thr_arg)
{
	...
	do {
		...
		wait_event_interruptible(wq, kthread_should_stop()
					 || atomic_read(&cond));//put itself in wq,
//and wait for kthread_should_stop() event or change of 'cond' variable
		if (atomic_read(&cond))
