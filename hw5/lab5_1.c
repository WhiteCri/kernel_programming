...
static void t_fun(unsigned long t_arg)
{
	...
	if (START == 1 && N == 1){
		pr_info("start checking interval\n");
		t1 = j_pre = jiffies;

		min_interval = jiffies;//must be smaller in the future
		max_interval = 0; // must be bigger in the future
		++N;
	}
	else if (N == 1000){
		t2 = jiffies;
		avg_interval = (t2 - t1)/1000;
		pr_info("min_interval, max_interval, avg_interval: %ld %ld %ld\n",
		...
		j_cur = jiffies;
		j_interval = j_cur - j_pre;
		if (j_interval > max_interval) max_interval = j_interval;
		if (j_interval < min_interval) min_interval = j_interval;
